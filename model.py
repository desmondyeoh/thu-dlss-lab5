...
class RNN(object):
    def __init__(self,
        ...
        cell_fw_li = [BasicRNNCell(num_units) for x in range(num_layers)]
        cell_bw_li = [BasicRNNCell(num_units) for x in range(num_layers)]
        
        cell_fw = MultiRNNCell(cell_fw_li)
        cell_bw = MultiRNNCell(cell_bw_li)
...
#             print("SHAPES", Ws2.shape, Ws1.shape, H.shape)
#             Ws2 150,10     [param_da, param_r]
#             Ws1 1024, 150  [2*num_units, param_da]
#             H 5, ?, 1024   (batch, length, 2*num_units)
            # A: ?
            # M: batch, param_r*2*num_units

            A = tf.nn.softmax(tf.einsum('ijk,kl->ijl', tf.tanh(tf.einsum('ijk,kl->ijl', H, Ws1)), Ws2))
            A = tf.Print(A, [A])
            
            M = tf.einsum('ijk,ijc->ikc',A,H)
            
            
            M = tf.reshape(M, (batch_size, param_r*2*num_units))
            
#             M = np.dot(A.transpose((0,2,1)), np)                    
        ...
        self.accuracy = tf.reduce_sum(tf.cast(tf.equal(self.labels, tf.cast(predict_labels, tf.int32)), tf.int32), name='accuracy')
